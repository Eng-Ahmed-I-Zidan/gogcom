<?php
  ...
  // [$HelpfulKnow]
  //if(isset($_POST['HelpfulKnow'])){ $HelpfulKnow = $_POST['HelpfulKnow']; } else { $HelpfulKnow = ""; }
  $HelpfulKnow = $_POST['HelpfulKnow'] ?? "";
  ...
  if($HelpfulKnow != ""){
    // [$commentid]
    //if(isset($_POST['commentid'])){ $commentid = $_POST['commentid']; } else { $commentid = ""; }
    $commentid = $_POST['commentid'] ?? "";

    // [$HelpOrNot]
    //if(isset($_POST['HelpOrNot'])){ $HelpOrNot = $_POST['HelpOrNot']; } else { $HelpOrNot = ""; }
    $HelpOrNot = $_POST['HelpOrNot'] ?? "";

    if($HelpOrNot == "Yes"){ $prepareForUpdate = "UPDATE " . $esn7a841 . " SET helpful_comment_for_all = (helpful_comment_for_all + 1), helpful_comment_for_this_user = 1 WHERE commentId = ?"; }
    else if($HelpOrNot == "No"){ $prepareForUpdate = "UPDATE " . $esn7a841 . " SET Un_helpful_comment_for_all = (Un_helpful_comment_for_all + 1), helpful_comment_for_this_user = 1 WHERE commentId = ?"; }

    $Update_comments = $connect->prepare($prepareForUpdate);
    $Update_comments->execute(array($commentid));
  } else {
    // [$InputVal]
    //if(isset($_POST['InputVal'])){ $InputVal = $_POST['InputVal']; } else { $InputVal = ""; }
    $InputVal = $_POST['InputVal'] ?? "";

    // [$textAreaVal]
    //if(isset($_POST['textAreaVal'])){ $textAreaVal = $_POST['textAreaVal']; } else { $textAreaVal = ""; }
    ...
    // [$GameIDInDB]
    //if(isset($_POST['GameIDInDB'])){ $GameIDInDB = $_POST['GameIDInDB']; } else { $GameIDInDB = ""; }
    $GameIDInDB = $_POST['GameIDInDB'] ?? "";
    ...
    // insert comments
    $Insert_comments = $connect->prepare("INSERT INTO " . $esn7a841 . " (comment_title, comment_text, game_rating, comment_date, comment_username, comment_gameid, comment_language) VALUES (:comment_title, :comment_text, :game_rating, :comment_date, :comment_username, :comment_gameid, :comment_language)");
    $Insert_comments->execute(array(
      "comment_title" => $InputVal,
      "comment_text" => $textAreaVal,
      "game_rating" => $rating,
      "comment_date" => $date,
      "comment_username" => $UserName,
      "comment_gameid" => $GameIDInDB,
    ...
    // for rating virified owner
    // FOR reating
    $sumOfCommentsRatingVerifiedOwner = 0;
    ...
    <div id="for_catalog__paginator_wrapper">
      ...
      foreach($fetch_comments as $fetch_comment){
        ...
        if($fetch_comments_user_information['Realy_in_gog'] == "1"){
          $sumOfCommentsRatingVerifiedOwner += (int)$fetch_comment['game_rating'];
      ...
      <?php } ?>
          <div class="review__item" <?php if($limitCommentsInOnePage == 1 && $ShowCommentPhyInPage == "Most-helpful"){ ?> style="background: hsla(0,0%,100%,.2);" <?php } ?> commentid="<?php $fetch_comment['commentId']; ?>">
    ...
    <!---->
    <div class="returnResalutForratingTextoverall ng-hide"><?php if($commentIndex > 0){ echo number_format(($sumOfCommentsRating / $commentIndex), 1, ".", ""); } else { echo '5'; } ?></div>
    <!---->
    <div class="returnResalutForratingTextVirifedOwner ng-hide"><?php if($commentIndexVerifiedOwner > 0){ echo number_format(($sumOfCommentsRatingVerifiedOwner / $commentIndexVerifiedOwner), 1, ".", ""); } else { echo '0'; } ?></div>
    <!---->
