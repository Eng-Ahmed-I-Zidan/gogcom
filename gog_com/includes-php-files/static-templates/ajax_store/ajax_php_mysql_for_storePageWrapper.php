<?php
  ...
  //if(isset($_POST['ReturnGamesIdForWrapper'])){ $ReturnGamesIdForWrapper = $_POST['ReturnGamesIdForWrapper']; } else { $ReturnGamesIdForWrapper = ""; }
  $ReturnGamesIdForWrapper = $_POST['ReturnGamesIdForWrapper'] ?? "";
  ...
  $ReturnGamesIdForWrapperAfterTrim = trim($ReturnGamesIdForWrapper);
  $ReturnGamesIdForWrapperAfterExplode = explode(',', $ReturnGamesIdForWrapperAfterTrim);

  for($loop = (($ngItemIndex - 1) * 48); $loop < ($ngItemIndex * 48); $loop++){
    if($loop == count($ReturnGamesIdForWrapperAfterExplode)){ break; }
    else {
      $select_game_by_id = $connect->prepare("SELECT * FROM " . $allGameInOnePlaceTable . " WHERE gc_g_id = ?");
      $select_game_by_id->execute(array($ReturnGamesIdForWrapperAfterExplode[$loop]));
      // select games
